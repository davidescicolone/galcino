...
export class MatchRepositoryImpl implements MatchRepository {
    ...
    async insertMatch(match: Match) {
        return (await MongoDB.getDB()).matches.insertOne(await this.dbMatchFrom(match))
    }

    async updateMatch(match: Match) {
        const dbMatch = await this.dbMatchFrom(match)
        await (await MongoDB.getDB()).matches!.replaceOne({_id: dbMatch._id}, dbMatch)
    }

    private async retrieveMatches(filter: Document): Promise<Match[]> {
        const dbMatches = (await (await MongoDB.getDB()).matches!.find(filter).toArray()) as MatchDAO[];
